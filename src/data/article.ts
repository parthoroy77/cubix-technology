import { Article } from "@/types";

export const articlesData: Article[] = [
  {
    title: "The Future of Artificial Intelligence",
    author: "John Doe",
    publishedAt: "2025-08-01",
    views: 12345,
    likes: 1023,
    comments: 450,
    tags: ["AI", "Technology", "Future", "Innovation"],
    category: "Tech",
    content:
      "In this article, we explore the future of AI and how it will shape the world. From autonomous machines to intelligent assistants, AI is already impacting industries and daily life.",
    readTimeMinutes: 8,
    id: 1,
    status: "published",
  },
  {
    title: "How to Master the Art of Cooking Pasta",
    author: "Jane Smith",
    publishedAt: "2025-07-25",
    views: 8500,
    likes: 768,
    comments: 120,
    tags: ["Cooking", "Pasta", "Recipe", "Food"],
    category: "Lifestyle",
    content:
      "This guide will take you through the essential steps to cook perfect pasta every time. From selecting the right type of pasta to pairing it with the ideal sauce.",
    readTimeMinutes: 5,
    id: 2,
    status: "published",
  },
  {
    title: "The Rise of Electric Vehicles in 2025",
    author: "Michael Green",
    publishedAt: "2025-06-30",
    views: 20500,
    likes: 1540,
    comments: 230,
    tags: ["Electric Vehicles", "Sustainability", "Automotive", "Green Technology"],
    category: "Automotive",
    content:
      "Electric vehicles are becoming more popular as both consumers and manufacturers embrace sustainable transportation options. This article dives deep into the latest advancements and trends.",
    readTimeMinutes: 10,
    id: 3,
    status: "published",
  },
  {
    title: "How to Start a Successful Blog in 2025",
    author: "Sarah Lee",
    publishedAt: "2025-08-05",
    views: 18200,
    likes: 1350,
    comments: 500,
    tags: ["Blogging", "Content Creation", "Entrepreneurship"],
    category: "Business",
    content:
      "Starting a blog is easier than ever, but turning it into a successful business requires the right approach. In this article, we break down the steps to grow your blog audience and monetize it.",
    readTimeMinutes: 7,
    id: 4,
    status: "published",
  },
  {
    title: "Exploring the Best Hiking Trails in the US",
    author: "Tom Harris",
    publishedAt: "2025-05-15",
    views: 9200,
    likes: 680,
    comments: 250,
    tags: ["Hiking", "Outdoors", "Travel", "Adventure"],
    category: "Travel",
    content:
      "Whether you're a seasoned hiker or a beginner, the US has some of the best trails to explore. From the Rockies to the Appalachian Mountains, this guide covers the top hiking spots.",
    readTimeMinutes: 9,
    id: 5,
    status: "published",
  },
  {
    title: "How Blockchain Will Change the Financial World",
    author: "Ella Carter",
    publishedAt: "2025-07-18",
    views: 14500,
    likes: 980,
    comments: 400,
    tags: ["Blockchain", "Finance", "Cryptocurrency", "Technology"],
    category: "Finance",
    content:
      "Blockchain technology is revolutionizing the financial sector. In this article, we discuss how blockchain can reduce fraud, streamline transactions, and enhance transparency in finance.",
    readTimeMinutes: 6,
    id: 6,
    status: "published",
  },
  {
    title: "10 Secrets to Boost Your Mental Health",
    author: "Alex Johnson",
    publishedAt: "2025-07-22",
    views: 7500,
    likes: 450,
    comments: 300,
    tags: ["Mental Health", "Self-care", "Wellness", "Psychology"],
    category: "Health",
    content:
      "Mental health is just as important as physical health. In this article, we cover 10 simple strategies that can help you maintain a positive mental state and improve overall well-being.",
    readTimeMinutes: 8,
    id: 7,
    status: "published",
  },
  {
    title: "The Best Gadgets of 2025",
    author: "David Kim",
    publishedAt: "2025-06-12",
    views: 13400,
    likes: 1210,
    comments: 530,
    tags: ["Gadgets", "Technology", "Reviews"],
    category: "Tech",
    content:
      "From foldable phones to smart home devices, this list covers the top gadgets of 2025. These innovations are changing the way we live and interact with technology.",
    readTimeMinutes: 7,
    id: 8,
    status: "published",
  },
  {
    title: "The Power of a Plant-Based Diet",
    author: "Rachel Adams",
    publishedAt: "2025-08-03",
    views: 11350,
    likes: 1020,
    comments: 280,
    tags: ["Plant-based", "Diet", "Health", "Sustainability"],
    category: "Health",
    content:
      "Plant-based diets are becoming more popular for their health benefits and sustainability. This article explains how adopting a plant-based diet can improve your health and reduce your environmental footprint.",
    readTimeMinutes: 6,
    id: 9,
    status: "published",
  },
  {
    title: "Mastering SEO in 2025: A Beginner's Guide",
    author: "Chris Williams",
    publishedAt: "2025-06-25",
    views: 16300,
    likes: 1450,
    comments: 350,
    tags: ["SEO", "Digital Marketing", "Content Strategy", "Business"],
    category: "Marketing",
    content:
      "SEO is an essential skill for anyone in digital marketing. This comprehensive guide provides the latest techniques to boost your website’s visibility on search engines.",
    readTimeMinutes: 10,
    id: 10,
    status: "published",
  },
  {
    title: "The Rise of Virtual Reality in Education",
    author: "Sophia Lee",
    publishedAt: "2025-07-10",
    views: 10200,
    likes: 900,
    comments: 150,
    tags: ["Virtual Reality", "Education", "Tech"],
    category: "Education",
    content:
      "Virtual reality (VR) is transforming the classroom experience. This article explores how VR is being used in education to enhance learning and engage students.",
    readTimeMinutes: 8,
    id: 11,
    status: "published",
  },
  {
    title: "The Ultimate Guide to Traveling on a Budget",
    author: "Emily Turner",
    publishedAt: "2025-05-20",
    views: 12200,
    likes: 1050,
    comments: 480,
    tags: ["Travel", "Budgeting", "Adventure", "Cheap Travel"],
    category: "Travel",
    content:
      "Traveling doesn’t have to break the bank. This guide provides top tips and resources to help you plan your dream vacation without spending too much.",
    readTimeMinutes: 9,
    id: 12,
    status: "published",
  },
  {
    title: "How to Achieve Financial Independence",
    author: "Olivia Brown",
    publishedAt: "2025-06-05",
    views: 15400,
    likes: 1300,
    comments: 400,
    tags: ["Finance", "Investing", "Personal Finance", "Money"],
    category: "Finance",
    content:
      "Financial independence is a goal for many. This article breaks down the key steps to take control of your money, invest smartly, and eventually achieve financial freedom.",
    readTimeMinutes: 8,
    id: 13,
    status: "published",
  },
  {
    title: "The Science of Happiness",
    author: "David Allen",
    publishedAt: "2025-07-30",
    views: 9800,
    likes: 780,
    comments: 215,
    tags: ["Happiness", "Psychology", "Well-being"],
    category: "Health",
    content:
      "What makes people happy? In this article, we look at the science behind happiness and how you can apply psychological principles to improve your well-being.",
    readTimeMinutes: 7,
    id: 14,
    status: "published",
  },
  {
    title: "How to Build a Personal Brand in 2025",
    author: "Laura Roberts",
    publishedAt: "2025-05-30",
    views: 11000,
    likes: 1120,
    comments: 460,
    tags: ["Personal Brand", "Career", "Social Media", "Business"],
    category: "Business",
    content:
      "Building a personal brand is more important than ever in 2025. This article offers insights on creating a strong personal brand that resonates with your audience and accelerates your career.",
    readTimeMinutes: 6,
    id: 15,
    status: "published",
  },
  {
    title: "The Best Ways to Stay Productive Working from Home",
    author: "James Foster",
    publishedAt: "2025-06-15",
    views: 9800,
    likes: 850,
    comments: 350,
    tags: ["Productivity", "Remote Work", "Home Office", "Workplace"],
    category: "Work",
    content:
      "Remote work is the new normal. Here are the best strategies and tools to stay productive while working from home and avoid distractions.",
    readTimeMinutes: 6,
    id: 16,
    status: "published",
  },
  {
    title: "Exploring the Wonders of Space",
    author: "Dr. Steven Parker",
    publishedAt: "2025-08-10",
    views: 15500,
    likes: 1245,
    comments: 550,
    tags: ["Space", "Astronomy", "Science", "Exploration"],
    category: "Science",
    content:
      "Space exploration is one of humanity’s greatest endeavors. In this article, we explore the latest missions, discoveries, and the future of space exploration.",
    readTimeMinutes: 9,
    id: 17,
    status: "published",
  },
  {
    title: "5 Tips for a Successful Career Change",
    author: "Megan Clark",
    publishedAt: "2025-07-13",
    views: 8300,
    likes: 760,
    comments: 190,
    tags: ["Career Change", "Personal Growth", "Motivation"],
    category: "Career",
    content:
      "Thinking about changing careers? Here are five actionable tips to help you navigate the transition smoothly and successfully.",
    readTimeMinutes: 7,
    id: 18,
    status: "published",
  },
  {
    title: "Understanding Cryptocurrency and Blockchain",
    author: "Gregory Mills",
    publishedAt: "2025-06-07",
    views: 13500,
    likes: 1100,
    comments: 310,
    tags: ["Cryptocurrency", "Blockchain", "Tech", "Finance"],
    category: "Tech",
    content:
      "This article explains the basics of cryptocurrency and blockchain technology, providing an introduction for beginners looking to dive into the world of digital currencies.",
    readTimeMinutes: 8,
    id: 19,
    status: "published",
  },
  {
    title: "The Art of Negotiation in Business",
    author: "Catherine Reynolds",
    publishedAt: "2025-05-18",
    views: 9500,
    likes: 890,
    comments: 170,
    tags: ["Negotiation", "Business", "Strategy", "Leadership"],
    category: "Business",
    content:
      "Negotiation is a key skill for any business leader. In this article, we share strategies for negotiating effectively and achieving win-win outcomes.",
    readTimeMinutes: 7,
    id: 20,
    status: "published",
  },
  {
    title: "How to Boost Your Immune System Naturally",
    author: "Lily Grant",
    publishedAt: "2025-06-28",
    views: 7800,
    likes: 640,
    comments: 290,
    tags: ["Health", "Immune System", "Wellness", "Natural Remedies"],
    category: "Health",
    content:
      "Your immune system plays a crucial role in keeping you healthy. Learn how to boost your immunity naturally with diet, exercise, and lifestyle changes.",
    readTimeMinutes: 6,
    id: 21,
    status: "published",
  },
  {
    title: "The Importance of Work-Life Balance",
    author: "Jessica Moore",
    publishedAt: "2025-07-08",
    views: 8200,
    likes: 720,
    comments: 220,
    tags: ["Work-Life Balance", "Stress Management", "Productivity"],
    category: "Career",
    content:
      "Achieving work-life balance can be challenging but essential for long-term well-being. Here are practical tips for finding balance in your professional and personal life.",
    readTimeMinutes: 6,
    id: 22,
    status: "draft",
  },
  {
    title: "The Power of Networking in 2025",
    author: "Natalie Davis",
    publishedAt: "2025-05-10",
    views: 13400,
    likes: 1220,
    comments: 430,
    tags: ["Networking", "Career", "Personal Growth", "Business"],
    category: "Business",
    content:
      "Networking is more important than ever in the digital age. This article discusses how to network effectively in 2025 and build meaningful professional relationships.",
    readTimeMinutes: 7,
    id: 23,
    status: "published",
  },
  {
    title: "How to Stay Motivated During Tough Times",
    author: "Samuel Clark",
    publishedAt: "2025-07-02",
    views: 9000,
    likes: 710,
    comments: 180,
    tags: ["Motivation", "Resilience", "Self-care"],
    category: "Wellness",
    content:
      "Staying motivated during difficult times is a challenge. This article offers practical advice to help you push through adversity and maintain your focus on your goals.",
    readTimeMinutes: 6,
    id: 24,
    status: "published",
  },
  {
    title: "The Rise of Remote Learning in 2025",
    author: "Sophia Brown",
    publishedAt: "2025-07-14",
    views: 10700,
    likes: 860,
    comments: 310,
    tags: ["Education", "Remote Learning", "Technology"],
    category: "Education",
    content:
      "The education system is evolving with the rise of remote learning. This article explores the benefits and challenges of virtual education in 2025 and how it is reshaping learning.",
    readTimeMinutes: 7,
    id: 25,
    status: "draft",
  },
  {
    title: "Understanding the Metaverse",
    author: "Lucas White",
    publishedAt: "2025-08-02",
    views: 9400,
    likes: 500,
    comments: 190,
    tags: ["Metaverse", "Tech", "Future", "Virtual Reality"],
    category: "Tech",
    content:
      "The Metaverse is becoming a major part of the digital world. This article breaks down what it is, how it works, and what the future holds.",
    readTimeMinutes: 6,
    id: 26,
    status: "published",
  },
  {
    title: "Building Resilient Teams in 2025",
    author: "Adam Harris",
    publishedAt: "2025-07-20",
    views: 8700,
    likes: 670,
    comments: 280,
    tags: ["Teamwork", "Leadership", "Business"],
    category: "Business",
    content:
      "Building strong, resilient teams is more crucial than ever. Learn strategies to foster collaboration and adapt to challenges in the modern work environment.",
    readTimeMinutes: 7,
    id: 27,
    status: "published",
  },
  {
    title: "Sustainable Fashion Trends in 2025",
    author: "Zara Wilson",
    publishedAt: "2025-06-26",
    views: 7500,
    likes: 450,
    comments: 180,
    tags: ["Fashion", "Sustainability", "Eco-friendly"],
    category: "Lifestyle",
    content:
      "Sustainability in fashion is changing the industry. In this article, we cover the most popular trends in eco-friendly fashion and what to look out for in 2025.",
    readTimeMinutes: 8,
    id: 28,
    status: "published",
  },
  {
    title: "How to Manage Stress in 2025",
    author: "Julia Grant",
    publishedAt: "2025-07-09",
    views: 9200,
    likes: 540,
    comments: 220,
    tags: ["Stress Management", "Mental Health", "Wellness"],
    category: "Health",
    content:
      "With modern life becoming more demanding, stress management is crucial. This article offers practical tips and techniques to help manage stress effectively.",
    readTimeMinutes: 6,
    id: 29,
    status: "published",
  },
  {
    title: "The Future of Work in a Remote-First World",
    author: "Ella Patel",
    publishedAt: "2025-06-18",
    views: 10600,
    likes: 1100,
    comments: 400,
    tags: ["Remote Work", "Future of Work", "Workplace Trends"],
    category: "Work",
    content:
      "The future of work is shifting toward remote-first. Learn about the challenges and opportunities that come with this new way of working.",
    readTimeMinutes: 8,
    id: 30,
    status: "published",
  },
  {
    title: "The Role of AI in Healthcare",
    author: "Dr. William Scott",
    publishedAt: "2025-07-16",
    views: 11700,
    likes: 950,
    comments: 300,
    tags: ["AI", "Healthcare", "Technology", "Medicine"],
    category: "Health",
    content:
      "Artificial intelligence is transforming healthcare. This article explains how AI is used in diagnostics, treatment planning, and patient care.",
    readTimeMinutes: 9,
    id: 31,
    status: "published",
  },
];
